...
def load_data():
    ...
    # Estandarizar nombres si vienen distintos desde el notebook
    rename_map = {}
...
def build_cluster_top_words(docs: pd.DataFrame, top_n: int = 20) -> pd.DataFrame:
    ...
    for cluster_id in sorted(docs["cluster"].dropna().unique()):
        ...
        for word, count in counts:
            rows.append(
                {"cluster": cluster_id, "word": word, "count": count}
            )
...
def main():
    ...
    cluster_words = words[words["cluster"] == selected_cluster].sort_values(
    ...
    show_temporal_tab = not yearly_counts.empty
    ...
    if show_temporal_tab:
        tab_names.append("Evolución temporal")
